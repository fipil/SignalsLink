...
def find_build_outputs(project_root: Path) -> tuple[Path, Path]:
    ...
    dll_candidates = sorted(debug_dir.glob(f"**/{modFile}.dll"), key=lambda p: p.stat().st_mtime, reverse=True)

    if not dll_candidates:
        raise FileNotFoundError(f"{modFile}.dll was not found under bin/Debug. Build may have failed.")
...
def run_tests(solution_root: Path) -> None:
    ...
    The bridge's own tests live in the shared test project, so the whole suite runs - a released
    archive nobody tested is worse than no archive.
    """
...
def main() -> int:
    project_root = Path(__file__).resolve().parent.parent / f"{modFile}"
    ...
    subprocess.run(
        ["dotnet", "build", f"{modFile}.csproj", "-c", "Debug"],
        cwd=project_root,
        check=True,
    )
    ...
    zip_path = solution_root / "Zips" / zip_name
