...
{
    /// <summary>
    /// Tells a vehicle that has stopped from one that is merely slow, by sampling positions - a
    /// vehicle publishes no speed.
    ///
    /// Two quiet samples, not one: a vehicle changing direction passes through a standstill.
    ///
    ...
    public class StandingWatch
    {
        ...
        public bool IsStanding(long entityId, double x, double y, double z, long now)
        {
            ...
            // Two docks may watch the same vehicle within one tick. Comparing a position with
            // itself would find it quiet, and a moving vehicle would be called stopped.
        ...
        /// <summary>
        /// Drops what has not been looked at for a while, or every vehicle that ever passed stays
        /// for the life of the world.
        ///
        /// By age, never by who asked. Clearing whatever one device could not see wiped the other
        /// device's vehicle on every tick, so neither ever gathered two quiet samples and no dock in
